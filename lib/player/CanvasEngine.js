...
export class CanvasEngine {
  ...
  open(at) {
    ...
    const url = at > 0 ? `${c.streamUrl}${c.streamUrl.includes('?') ? '&' : '?'}t=${at.toFixed(3)}` : c.streamUrl;
    this.worker.postMessage({ type: 'open', url, startTime: c.startTime, seekTo: at, hasAudio: c.hasAudio && !!this.ctx, duration: c.duration });
  }
