...
export function extractYoutubeStream(videoId) {
  ...
  return cached('stream', videoId, 60 * 60e3, async () => {
    try {
      const json = execSync(`yt-dlp -j --no-warnings "https://www.youtube.com/watch?v=${videoId}"`, { encoding: 'utf-8', timeout: 45000, maxBuffer: 10 * 1024 * 1024 });
      const info = JSON.parse(json);
      
      // Prefer format 18 (360p mp4, widely available), fall back to first format
