...
// WebCodecs -> OffscreenCanvas + Web Audio). YouTube videos are extracted via ytdl-core.
...
import { VIDEO_ID_RE, extractYoutubeStream } from './youtube.js';
...
export async function createPlaybackSession(source) {
  ...
  if (source.kind === 'youtube') {
    ...
    // Extract stream directly from YouTube via ytdl-core.
    const extracted = await extractYoutubeStream(videoId);
    const s = createSession({ player: 'canvas', youtubeVideoId: videoId, youtubeUrl: extracted.url });
    return envelope(s, { title: source.title || extracted.title, youtube: { videoId },
      canvas: {
        ...
        video: { codec: 'h264' },
        audio: { codec: 'aac' },
        transcoding: { video: true, audio: true },
        seekable: false,
        streamUrl: `/api/youtube/stream/${videoId}?session=${encodeURIComponent(s.id)}`,
      },
      notice: 'Streaming YouTube video through the canvas player.' });
  }
